...
/// Only use to catch all error from the [goTestFile] function
export async function goTestFileWrapper(line?: number, source?: string) {
  try {
    goTestFile(line, source);
  } catch (err) {
    analyticsService.trackError(err);
  }
}

async function goTestFile(line?: number, source?: string) {
  ...
  if (!filePath) {
    vscode.window.showInformationMessage(
      'Error: unable to find path for current editor.',
    );
    return;
  }
  ...
  if (!filePath.startsWith(workspacePath)) {
    vscode.window.showInformationMessage(
      'Error: file not in current workspace.',
    );
    return;
  }
  ...
  if (!exist) {
    ...
    const options = [
      createLabel,
      cancelLabel,
      ...closest.map((file: { target: String; rating: number }) => {
    ...
    if (selection === cancelLabel || !options.includes(selection ?? '')) {
      analyticsService.tagEvent('open-test-file-cancel', {
    ...
    if (selection === createLabel) {
      ...
      analyticsService.tagEvent('create-test-file', {
        source: source ?? 'command',
      });
